...
public static void calculator(String path) throws Exception{
  //  BufferedReader br = new BufferedReader(new FileReader(new File(path))); 
	...
	*/
    ...
    System.out.println(Math.pow(Math.E, -5.0*0.3));
    for(int i=0;i<100;i++){
	Classifier cls = (Classifier) weka.core.SerializationHelper.read(path+"models/Classifier"+i+".model");	
	Classifier cls_w = (Classifier) weka.core.SerializationHelper.read(path+"w_models/Classifier"+i+".model");	
	if(!cls.toString().equals(cls_w.toString())){
